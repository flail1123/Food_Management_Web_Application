...
def products(request):
    if not request.user.is_authenticated:
        return render(request, 'not_logged_in.html', {'activeTab': 'products'})
...
def planToDisplay(request, displayedDate):
    ...
    isBasic = BasicPlan.objects.filter(user_id__id=request.user.id)
    if isBasic:
        basicFilter = Q()
        basicFilter = basicFilter | Q(id=isBasic[0].plan_id.id)
...
def calendar(request):
    if not request.user.is_authenticated:
        return render(request, 'not_logged_in.html', {'activeTab': 'calendar'})
...
def plans(request, displayedPlanId=-1, deleteId=-1):
    if not request.user.is_authenticated:
        return render(request, 'not_logged_in.html', {'activeTab': 'plans'})
...
def lists(request, deleteId=-1):
    if not request.user.is_authenticated:
        return render(request, 'not_logged_in.html', {'activeTab': 'lists'})
    return foodSets(request, "shoppingLists", int(deleteId))


def recipes(request, deleteId=-1):
    if not request.user.is_authenticated:
        return render(request, 'not_logged_in.html', {'activeTab': 'recipes'})
...
def meals(request, deleteId=-1):
    if not request.user.is_authenticated:
        return render(request, 'not_logged_in.html', {'activeTab': 'meals'})
...
def stats(request):
    if not request.user.is_authenticated:
        return render(request, 'not_logged_in.html', {'activeTab': 'stats'})
